...
public:
  ...
  void setJECSubjet(const std::variant<correction::Correction::Ref, correction::CompoundCorrection::Ref>&& jesSF) {
    ...
    if (auto corrObj = std::get_if<correction::Correction::Ref>(&jesSF)) {
      ...
      m_jecInputAreaRhoPhiRunSubjet = ((*corrObj)->inputs().size() == 6);
    } else {
      ...
      m_jecInputAreaRhoPhiRunSubjet = (std::get<correction::CompoundCorrection::Ref>(jesSF)->inputs().size() == 6);
    }
...
protected:
  ...
  bool m_jecInputAreaRhoSubjet{false}, m_jecInputAreaRhoPhiSubjet{false}, m_jecInputAreaRhoPhiRunSubjet{false}, m_doJECSubjet{false};
};
