...
const float btagUParT_tightWP_2024          = 0.4648;
...
const float btagUParT_mediumWP_2024         = 0.1272;
...
const float btagUParT_looseWP_2024          = 0.0246;
...
RVec<int> GetJetBTag(rvec_i GoodJet, rvec_f Jet_btagDeepB, int year, bool EE, int wp){
  // WP legend: 0->loose, 1->medium, 2->tight
    ...
    if(year == 2018){
      ...
      if(wp==2){
        bthres = btagUParT_tightWP_2024;
      }
      else if(wp==1){
        bthres = btagUParT_mediumWP_2024;
      }
      else if(wp==0){
        bthres = btagUParT_looseWP_2024;
      }
    }
